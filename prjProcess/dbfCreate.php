...
function functionCheckIfDatabaseExists($connection, $sql, $dbname, $flagcheck)
{
    $flag = false;
...
function populateTable($conn, $sql, $flagstatus)
{
    ...
    foreach ($membershipArray as $membership) {
        $sql =
            ...
            $membership[2] .
            "')";
        runQuery($sql, "New record insert $membership[0]", false, $flagstatus);
    }
    ...
    foreach ($signInIdArray as $signInId) {
        ...
        runQuery($sql, "New record insert $signInId[0]", false, $flagstatus);
    }
    ...
    foreach ($employeeArray as $employee) {
        ...
        runQuery($sql, "New record insert $employee[0]", false, $flagstatus);
    }
    ...
    while ($row = $result->fetch_assoc()) {
        getSigninid($conn, "employee", $row["firstName"], $row["lastName"]);
    }
    ...
    foreach ($memberArray as $member) {
        ...
        runQuery($sql, "New record insert $member[0]", false, $flagstatus);
    }
    ...
    $scheduleArray = [
        ["2023-11-10", "Hank", "Parker", "Mike", "Johnson", "WeightLifting"],
    ...
    foreach ($scheduleArray as $schedule) {
        ...
        runQuery($sql, "New record insert $schedule[0]", false, $flagstatus);
    }
    //schedule query
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        } elseif ($row["firstName"] == "Coco" || $row["firstName"] == "Hank") {
            setMembershipId(
                $conn,
                "member",
                $row["firstName"],
      ...
      function displaySchedule($conn)
      {
          ...
          if (isset($_POST["delete"])) {
              ...
              foreach ($result as $deleteId) {
                  mysqli_query(
                      $conn,
                      "DELETE FROM `schedule` WHERE scheduleId =$deleteId"
                  );
              }
          }
      } //display schedule

      function displayScheduleUser($conn)
      {
          $sql = "Select * from schedule;";
          $result = $conn->query($sql);
